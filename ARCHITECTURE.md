...
- The manifest also declares `id`/`scope` (the whole origin — this is a user-site at the domain root) and `handle_links: 'preferred'`. There is deliberately **no** `launch_handler`: the share target is a POST the service worker intercepts, and a non-default client mode could change whether that POST is delivered.
- **Getting a capture into the app window** takes a protocol handler, not link capturing. Chrome only captures navigations that create a new frame *without* an auxiliary browsing context, and `window.open` — what a bookmarklet does — is precisely an auxiliary context, so `/?capture&…` always landed in a tab (confirmed on Chrome desktop/Win11). The manifest therefore registers `web+gtd:`, and Settings offers two bookmarklets: the **app** one assigns `web+gtd:capture?title=…&url=…` to `location.href` (a custom scheme hands off to the OS handler and leaves the captured page in place), the **tab** one keeps the old `window.open`. Both land in `useUrlCapture`, which sanitises either shape identically and scrubs the address bar before any async work (ACR-004).
