...
- `taskList.name`; `task.title/description/link/linkTitle/links/discussionLog`;
  ...
  and `mindmapFolder.name` / `mindmap.name` / `mindmapNode.label` (Mindmaps —
  see below).
...
- **Encrypted:** folder/map **names** and node **labels** (the only content
  fields), in `_enc` on the wire and at rest under Paranoid (the at-rest
  middleware covers the three new tables). Labels may contain a markdown subset.
...
- **Device-local UI state:** collapse/expand state lives in localStorage
  (`gtd25-mindmap-ui`) as **opaque node ids only** — no content. It is removed
  by "Wipe All Data" and swept by the panic wipe's `gtd25-*` prefix clear.
