...
- **Paranoid ON — 🟠 content protected to passphrase strength; metadata leaks.**
  ...
  - **Mitigations in place:** Argon2id KDF, security-key tier (recommended for
    ...
    complete instead of silently leaving data behind. Since 2026-07-27 the wipe
...
- **Reliability / no-trace (verified by tests):** all crypto runs in memory,
...
- **Deniability of the *feature*** rests on: slot 2 being uniformly present; the
  ...
  vocabulary is deliberately avoided — no "duress"/"decoy"/"panic" strings or
  symbol names ship (verified by grepping `dist`); the neutral surface is a
  "secondary passphrase". This does **not** hide the capability from someone who
...
- **Honest residuals:**
  ...
  - **Not forensic-proof.** IndexedDB does not securely erase overwritten pages,
    ...
    forensic comparison.
  - **Consistency tells.** "Sync never set up here" and "no security key
    enrolled" are mild inconsistencies a sophisticated adversary might probe.
...
'wasm-unsafe-eval'`; `connect-src 'self' https://api.github.com`; `object-src 'none'`;
`base-uri 'self'`; styles allow `'unsafe-inline'` for runtime-injected Tailwind). This
raises the bar for injected-script and exfiltration attacks but cannot stop an attacker
who can replace the served bundle itself. It is build-only (the dev server needs
inline/eval for HMR).
