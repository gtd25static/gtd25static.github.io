...
- **Web Share Target (Android "share to GTD25").** The share target is **POST /
  ...
  **body, not the URL** — this removes the earlier GET-in-query-string exposure
  ...
  the share window. Mitigation: shares are user-initiated; unlock promptly, or don't
  share into a locked Paranoid device. (The bookmarklet capture still uses a GET
