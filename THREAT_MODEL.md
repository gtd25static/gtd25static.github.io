...
- **ON:** local content + credentials are encrypted at rest behind the vault
  ...
  prompt) are stored only in the vault. An idle/lock screen gates access; failed-attempt and panic wipes exist.
...
- **Deniability of the *feature*** rests on: slot 2 being uniformly present; the
  ...
  names ship (verified by grepping `dist`; "panic" does appear, but only as the
...
- **Honest residuals:**
  ...
  - **Consistency tells.** "Sync never set up here", "no security key
    ...
    inconsistencies a sophisticated adversary might probe. So is timing: the
