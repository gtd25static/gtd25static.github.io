...
- **Web Share Target (Android "share to GTD25").** The share target is **POST /
  ...
  **body, not the URL** — this removes the earlier GET-in-query-string exposure
  ...
  the share window. Worse, consumption is keyed off the `?shareTarget` redirect URL:
  if that navigation is lost (tab closed while locked, app next opened from the
  launcher), the plaintext stash is **never swept and persists indefinitely**; the SW
  also stashes shared files **without any size cap** (the 30 MB Shared Folder quota
  applies only at consume time). Both are tracked as open findings ACR-017/ACR-018 in
  `codex_appsec_review.md`. Mitigation: shares are user-initiated; unlock promptly, or
  don't share into a locked Paranoid device. (The bookmarklet capture still uses a GET
