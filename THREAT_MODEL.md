...
- **`taskList.archivedAt`** — when a list was archived (absent = active), same
  ...
  Trash window and local backups. Since 2026-09-25 **`completedAt`, `status` and
  a follow-up's `archived` (with its field timestamp) drive the same kind of
  expiry**: a task completed, or a follow-up resolved, over 12 months ago goes to
  the Trash at startup. Back-dating them on a writable backend has the same
  effect and the same standing — such a writer can already set `deletedAt` —
...
- **Shared, file-derived wipe lifecycle.** Each trusted (approver) device derives a
  ...
  - **"Last seen" (since 2026-09-25).** While **unlocked**, a protected device
    re-publishes its own registry entry at most once a day (it needs the sync-derived
    MAC key, so never while locked); its trusted devices show the entry's MAC-verified
    `updatedAt` as "last seen", and after 14 days of silence "no activity since …".
    A backend writer can't fake a later time (the MAC covers it). **This is how a
    duress wipe shows up, by design (user's choice among three options):** a device
    wiped by its secondary passphrase sends *nothing* — no status, no removal — it
    just stops refreshing, exactly like a device left unused or lost, so neither the
    repo nor another device reveals that a coercion or a wipe happened (Scenario 3b
    and the "stays off GitHub" guarantee are unchanged). The owner removes it with the
    existing wipe-then-forget. **Residual:** the entry's plaintext `updatedAt`, and the
    git history of `gtd25-devices.json`, now show a reader of the backend (Scenario 7)
    on which days each protected device was unlocked with sync — day granularity, and
    largely visible before from its own sync pushes on the days it edited something.
