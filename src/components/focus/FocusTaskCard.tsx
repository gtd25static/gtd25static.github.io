...
/**
 * Read-only Focus Mode card. Deliberately a commitment device: no editing, no
 * navigation, no expand — the only ways out are Complete and Delete (plus
 * starting a Pomodoro in place).
 */
export function FocusTaskCard({ task }: { task: Task }) {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700/60 dark:bg-zinc-900/50">
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <button
          onClick={() => focusTask(task.id, { startTimer: true, navigate: false })}
