...
export function CheckForUpdatesButton({ onActivate }: Props) {
  ...
  return (
    <button
      ...
      className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-full text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 disabled:opacity-60 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-200 md:min-h-0 md:min-w-0 md:p-2"
    >
      ...
      {/* Icon-only in the sidebar's bottom row; the label stays for screen readers. */}
      <span className="sr-only">{checking ? 'Checking…' : 'Check for app updates'}</span>
    </button>
  );
}
