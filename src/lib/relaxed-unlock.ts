...
// trailing 36h is the unavoidable baseline, and each unlock beyond it adds +10%,
...
export const RELAXED_WINDOW_MS = 36 * 60 * 60 * 1000;

/** Count unlock timestamps within the trailing RELAXED_WINDOW_MS of `now` (order-independent). */
export function unlocksInWindow(history: number[], now: number): number {
 ...
 * the evaluation window, capped at ×2. So 0–1 unlocks → ×1.0 (the first just gets
 * you in), 2 → ×1.10, … , 11 or more → ×2.0.
 */
export function computeUnlockMultiplier(history: number[], now: number): number {
  const additional = Math.max(0, unlocksInWindow(history, now) - 1);
...
/** Drop unlock timestamps outside the trailing evaluation window (keeps the list small). */
