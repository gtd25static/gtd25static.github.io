...
import { EXIT_MS, LAYOUT_MS, easeGlide, lerpLayout, prefersReducedMotion } from '../lib/mindmap-motion';
 ...
 *
 * Every transition takes the same LAYOUT_MS, whatever its size: a two-node
 * toggle and a whole-map unfold read as one gesture. (There used to be a
 * per-transition scale that shortened big reveals; it made the everyday small
 * expands the slowest thing on screen.)
 *
